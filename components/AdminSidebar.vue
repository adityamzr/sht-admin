...
const mediaMenu: Item[] = [
    ...
    {
        label: "Kontribusi Pengguna",
        to: "/media/contributions",
        icon: Inbox,
        section: "INTERAKSI",
    },
    {
        label: "Feedback Artikel",
...
const activeTo = computed(() => {
    const matches = menu.value.filter((x) =>
        x.to === "/"
            ? route.path === "/"
            : route.path === x.to || route.path.startsWith(`${x.to}/`),
    );
...
<template>
    ...
    >
        <div
            class="flex h-16 items-center gap-2.5 border-b border-neutral-line px-5"
        >
            <img
            ...
            <div class="leading-tight">
                <p class="font-heading text-sm font-semibold text-brand-green">
                    Sudut Haramain
                </p>
                <p
            ...
            <template v-for="(item, index) in menu" :key="item.label"
                ><p
                    v-if="
                        item.section &&
                        (index === 0 ||
                            menu[index - 1].section !== item.section)
                    "
                ...
                <NuxtLink
                    ...
                    :class="
                        activeTo === item.to
                            ? 'bg-sht-olive-dark text-white hover:bg-sht-olive-dark hover:text-white'
                            : 'text-neutral-charcoal/70 hover:bg-sht-olive/5 hover:text-sht-olive'
                    "
                    ><component
                        :is="item.icon"
