...
async function load() {
    try {
        const [a, s] = await Promise.all([
            ...
            $fetch<any>("/api/admin/media/page-settings/makkah"),
        ]);
...
async function save() {
    saving.value = true;
    try {
        await $fetch("/api/admin/media/page-settings/makkah", {
            method: "PATCH",
            body: {
        ...
        dirty.value = false;
        showGlobalToast("Pengaturan Makkah berhasil disimpan.", "success");
    } catch (e: any) {
...
<template>
    <div>
        <PageHead title="Settings · Makkah" subtitle="Media · Page Settings"
            ><template #actions
                ><span
                    v-if="dirty"
            ...
            >
                ...
                <div class="mt-4 rounded-2xl bg-sht-olive p-6 text-white">
                    <h3 class="font-heading text-2xl font-bold">
                        Jelajahi Makkah dari satu tempat.
                    </h3>
