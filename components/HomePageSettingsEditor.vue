...
type Slot = "featured" | "supporting" | "explore";
type HeroTopic = {
    id: string;
    label: string;
    isActive: boolean;
...
const articles = ref<any[]>([]),
    ...
    heroTopics = ref<HeroTopic[] | null>(null),
    newTopic = ref(""),
    picker = ref<any>(null),
    search = ref(""),
    preview = ref(false),
    resetAllConfirm = ref(false),
...
const topicPageNumber = ref(1);
const topicPageCount = computed(() =>
    Math.max(1, Math.ceil(topicRows.value.length / topicPageSize)),
);
const topicPage = computed(() =>
    topicRows.value
        .map((topic, index) => ({ topic, index }))
        .slice(
            (topicPageNumber.value - 1) * topicPageSize,
            topicPageNumber.value * topicPageSize,
        ),
);
...
const pickList = computed(() =>
    picker.value === "supporting" ? supporting.value : explore.value,
);
...
function addTopic() {
    ...
    if (
        !label ||
        heroTopics.value?.some(
            (x) => x.label.toLowerCase() === label.toLowerCase(),
        )
    )
        return;
    if (heroTopics.value === null)
        heroTopics.value = canonicalTopics.map((x, i) => ({
            id: "default-" + i,
            label: x,
            isActive: true,
            sortOrder: i * 10,
        }));
    heroTopics.value.push({
        id: "topic-" + Date.now(),
        label,
        isActive: true,
        sortOrder: heroTopics.value.length * 10,
    });
    newTopic.value = "";
    mark();
}
function removeTopic(i: number) {
    heroTopics.value?.splice(i, 1);
    mark();
}
function toggleTopic(i: number) {
    if (heroTopics.value)
        heroTopics.value[i].isActive = !heroTopics.value[i].isActive;
    mark();
}
function moveTopic(i: number, d: number) {
    if (!heroTopics.value) return;
    const j = i + d;
    if (j < 0 || j >= heroTopics.value.length) return;
    [heroTopics.value[i], heroTopics.value[j]] = [
        heroTopics.value[j],
        heroTopics.value[i],
    ];
    heroTopics.value.forEach((x, n) => (x.sortOrder = n * 10));
    mark();
}
function resetTopics() {
    heroTopics.value = null;
    topicPageNumber.value = 1;
    mark();
}
function setTopicPage(page: number) {
    topicPageNumber.value = Math.min(Math.max(page, 1), topicPageCount.value);
}
watch(topicPageCount, () => setTopicPage(topicPageNumber.value));
function add(id: number) {
    if (!picker.value) return;
    const target = pickList.value;
    if (
        !target.includes(id) &&
        (picker.value === "supporting" ? target.length < 3 : target.length < 6)
    ) {
        target.push(id);
        mark();
    }
...
function remove(slot: Slot, id: number) {
    if (slot === "featured") featured.value = null;
    else {
        const target = slot === "supporting" ? supporting.value : explore.value;
        const i = target.indexOf(id);
        if (i >= 0) target.splice(i, 1);
    }
    mark();
}
function move(slot: "supporting" | "explore", i: number, d: number) {
    const target = slot === "supporting" ? supporting.value : explore.value;
    const j = i + d;
...
async function save() {
    ...
    try {
        ...
        error.value = e.data?.statusMessage || "Settings gagal disimpan.";
    } finally {
        saving.value = false;
    }
}
...
function title(id: number | null) {
    return id
        ? item(id)?.title || "Artikel tidak lagi terbit"
        : "Otomatis berdasarkan artikel terbit";
}
onMounted(load);
</script>
<template>
    <div>
        <PageHead title="Settings · Home" subtitle="Media · Page Settings"
            ><template #actions
                ><span
                    v-if="dirty"
        ...
        <div
            v-if="pending"
            class="mt-8 rounded-2xl border bg-white p-8 text-sm"
        >
        ...
        <div v-else class="mt-8 space-y-5">
            ...
            >
                <div class="flex items-center justify-between">
                    <div>
                        <h2 class="font-heading text-lg font-semibold">Hero</h2>
                        <p class="mt-1 text-sm text-neutral-charcoal/55">
                            Editable · hanya konten editorial, template tetap
                            dikelola sh-web.
                        </p>
                    </div>
                    <span
                        class="rounded-full bg-gold-sand px-3 py-1 text-[10px] font-semibold uppercase"
                        >Editable</span
                    >
                </div>
                <div class="mt-3 flex flex-wrap gap-2">
                    <button
                        ...
                        @click="resetHeroField('headline')"
                    >
                        Reset headline</button
                    ><button
                        class="text-xs underline"
                        @click="resetHeroField('subheadline')"
                    >
                        Reset subheadline</button
                    ><button
                        class="text-xs underline"
                        @click="resetHeroField('image')"
                    >
                        Reset image</button
                    ><button
                ...
                <div class="mt-5 grid gap-4 lg:grid-cols-[240px_1fr]">
                    <MediaImageUploader
                        v-model="hero.imageUrl"
                        @update:file-id="
                            hero.imageFileId = $event;
                            mark();
                        "
                ...
                <div
                    class="mt-5 rounded-xl border border-neutral-line bg-neutral-soft/40 p-4"
                >
                    <div class="flex items-center justify-between gap-3">
                        <div>
                            <h3 class="font-semibold">Topik Hero</h3>
                            <p class="mt-1 text-xs text-neutral-charcoal/55">
                                Override opsional. Tanpa override, memakai
                                daftar default sh-web.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <div v-if="localeTab === 'id'" class="mt-3 flex gap-2">
                        ...
                        /><button
                            type="button"
                            class="inline-flex items-center gap-1 rounded-full bg-sht-olive px-3 py-2 text-xs font-semibold text-white"
                            @click="addTopic"
                        >
                    ...
                    <div class="mt-3 space-y-2">
                        <div
                            ...
                            :key="entry.topic.id"
                            class="flex min-w-0 items-center gap-2 rounded-lg border bg-white p-2"
                        >
                            <button
                                type="button"
                        ...
                        >
                            <button
                                type="button"
                                class="rounded-full border px-3 py-1 text-xs disabled:opacity-40"
                                :disabled="topicPageNumber === 1"
                                @click="setTopicPage(topicPageNumber - 1)"
                            >
                                Sebelumnya</button
                            ...
                            ><button
                                type="button"
                                class="rounded-full border px-3 py-1 text-xs disabled:opacity-40"
                                :disabled="topicPageNumber === topicPageCount"
                                @click="setTopicPage(topicPageNumber + 1)"
                            >
                                Berikutnya
                            </button>
            ...
            >
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-neutral-charcoal/55">
                            Editable · kurasi artikel yang muncul di section
                            Sorotan.
                        </p>
                    </div>
                    <span
                        class="rounded-full bg-sht-olive/10 px-3 py-1 text-[10px] font-semibold uppercase text-brand-green"
                        >Editable</span
                    >
                </div>
                <button
                ...
                >
                    Reset Sorotan ke Otomatis
                </button>
                <div class="mt-5 grid gap-4 lg:grid-cols-[1fr_1fr]">
                    <div class="rounded-xl border p-4">
                        <p
                            class="text-xs font-semibold uppercase tracking-wider text-neutral-charcoal/50"
                        >
                            Featured
                        </p>
                    ...
                    <div class="rounded-xl border p-4">
                        <p
                            class="text-xs font-semibold uppercase tracking-wider text-neutral-charcoal/50"
                        >
                            Supporting · maksimal 3
                        </p>
                        <div
                            v-for="(id, i) in supporting"
                            :key="id"
                        ...
                        >
                            ...
                            ><button
                                class="text-xs"
                                :disabled="i === 0"
                                @click="move('supporting', i, -1)"
                            >
                                Naik</button
                            ><button
                                class="text-xs"
                                :disabled="i === supporting.length - 1"
                                @click="move('supporting', i, 1)"
                            >
                                Turun</button
                            ><button
                                class="text-xs text-red-700"
                                @click="remove('supporting', id)"
                            >
                                ×
                            </button>
                        </div>
                        <button
                            class="mt-3 rounded-full border px-3 py-1.5 text-xs"
                            @click="picker = 'supporting'"
                        >
                            Tambah supporting
                        </button>
                    </div>
            ...
            >
                <div class="flex items-center justify-between">
                    <div>
                        <h2 class="font-heading text-lg font-semibold">
                            Jelajahi Sudut Haramain
                        </h2>
                        <p class="mt-1 text-sm text-neutral-charcoal/55">
                            Editable · artikel editorial pilihan, maksimal 6.
                        </p>
                    </div>
                    <span
                        class="rounded-full bg-sht-olive/10 px-3 py-1 text-[10px] font-semibold uppercase text-brand-green"
                ...
                <div class="mt-4 space-y-2">
                    <div
                        v-for="(id, i) in explore"
                        :key="id"
                        class="flex items-center gap-2 rounded-xl border p-3 text-sm"
                    >
                        <span class="w-5 text-center text-xs">{{ i + 1 }}</span
                        ><span class="min-w-0 flex-1 truncate">{{
                            item(id)?.title
                        }}</span
                        ><button
                            class="text-xs"
                            :disabled="i === 0"
                            @click="move('explore', i, -1)"
                        >
                            Naik</button
                        ><button
                            class="text-xs"
                            :disabled="i === explore.length - 1"
                            @click="move('explore', i, 1)"
                        >
                            Turun</button
                        ><button
                            class="text-xs text-red-700"
                            @click="remove('explore', id)"
                        >
                            ×
                        </button>
                    </div>
            ...
            <button
                class="mr-2 rounded-full border border-red-200 px-5 py-2.5 text-sm font-semibold text-red-700"
            ...
            >
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-xs text-neutral-charcoal/55">
                            Hanya PUBLISHED dan tidak duplikat.
                        </p>
                    </div>
                ...
                <div class="mt-4 min-h-0 space-y-2 overflow-y-auto">
                    <button
                        ...
                        :disabled="
                            picker === 'featured'
                                ? a.id === featured
                                : pickList.includes(a.id)
                        "
                        class="flex w-full items-center gap-3 rounded-xl border p-3 text-left disabled:opacity-40"
                        @click="
                            picker === 'featured'
                                ? ((featured = a.id), mark(), (picker = null))
                                : add(a.id)
                        "
                    >
                        <img
        ...
        <div
            ...
            class="fixed inset-0 z-[60] flex items-center justify-center bg-neutral-charcoal/40 p-5"
        >
            <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
                ...
                <p class="mt-2 text-sm text-neutral-charcoal/65">
                    Override Hero, Sorotan, dan Jelajahi akan dihapus setelah
                    Anda menyimpan.
                </p>
        ...
        >
            <section
                class="flex max-h-[95vh] w-full max-w-6xl flex-col rounded-2xl bg-neutral-soft p-4 shadow-xl"
            >
                <div class="flex items-center justify-between">
                    <h2 class="font-heading text-lg font-semibold">
                        Preview Home · draft belum disimpan
                    </h2>
                    <div class="flex gap-2">
                        ...
                        >
                            Mobile</button
                        ><button class="text-2xl" @click="preview = false">
                            ×
                        </button>
                    </div>
                </div>
                <div
                    class="mt-4 min-h-0 overflow-auto rounded-xl bg-white p-6"
                    :class="
                        device === 'mobile'
                ...
                >
                    <div
                        class="overflow-hidden rounded-2xl bg-sht-olive text-white"
                    >
                        <img
                            v-if="hero.imageUrl"
                            :src="hero.imageUrl"
                            class="h-48 w-full object-cover opacity-70"
                        />
                        <div
                    ...
                    <h3 class="mt-8 font-heading text-2xl font-semibold">
                        Topik Hero
                    </h3>
                    <div
                        class="mt-3 space-y-1 text-sm text-neutral-charcoal/70"
                    >
                        <p v-for="topic in previewTopics" :key="topic">
                            {{ topic }}
                        </p>
                        <p v-if="!previewTopics.length">
                            Tidak ada topik aktif.
                        </p>
                    </div>
                    <h3 class="mt-8 font-heading text-2xl font-semibold">
                        Sorotan
