...
const { data } = await useAdminFetch<{ data: EstimationListItem[] }>(
...
async function openDetail(id: number) {
    ...
    try {
        const res = await $fetch<{ data: EstimationDetail }>(
            `/api/admin/estimations/${id}`,
            {
...
<template>
    <div>
        <PageHead
            title="Estimations"
            subtitle="Snapshot historis estimasi (read-only) — tidak berubah walau harga/kurs/produk berubah."
        />
            ...
            <table class="w-full min-w-[760px] text-left text-sm">
                ...
                >
                    <tr>
                        ...
                        <th class="px-5 py-3">Keberangkatan</th>
                        <th class="px-5 py-3">Durasi</th>
                        <th class="px-5 py-3">Total</th>
                        <th class="px-5 py-3">Per orang</th>
                ...
                <tbody class="divide-y divide-neutral-line">
                    <template v-for="e in rows" :key="e.id">
                        <tr
                            :class="
                                detail?.id === e.id ? 'bg-brand-sky/20' : ''
                            "
                        >
                            <td
                                class="px-5 py-3 font-semibold text-brand-green"
                            >
                                {{ e.estimationNumber }}
                            </td>
                            <td class="px-5 py-3">
                                {{ e.pilgrims }} jamaah · {{ e.departureCity }}
                            </td>
                            <td class="px-5 py-3 text-neutral-charcoal/60">
                                {{ e.departureDate }}
                            </td>
                            <td class="px-5 py-3 text-neutral-charcoal/60">
                                {{ e.durationDays }} hari (Makkah
                                {{ e.makkahNights }} · Madinah
                                {{ e.madinahNights }})
                            </td>
                                ...
                                <template v-else>
                                    <ul class="space-y-2">
                                        <li
                                            ...
                                            :key="item.id"
                                            class="flex flex-wrap items-baseline justify-between gap-2 rounded-xl border border-neutral-line bg-white px-4 py-2.5 text-sm"
                                        >
                                            <span>
                                                ...
                                                <span
                                                    v-if="item.detail"
                                                    class="block text-xs text-neutral-charcoal/50"
                                                    >{{ item.detail }}</span
                                                >
                                            </span>
                                            <span
                                    ...
                                    <div class="mt-3 flex flex-wrap gap-2">
                                        <span
                                            v-for="r in detail.rates"
                                            :key="
                                                r.sourceCurrency +
                                                r.targetCurrency
                                            "
                                            class="rounded-full bg-brand-sky/50 px-3 py-1 text-xs font-semibold text-brand-green"
                                        >
                                            Kurs snapshot:
                                            {{ r.sourceCurrency }} →
                                            {{ r.targetCurrency }} @
