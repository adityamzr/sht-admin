...
const query = computed(() => ({
  ...
  vendorType: vendorType.value || undefined,
  status: status.value || undefined,
  page: page.value,
  pageSize: pageSize.value,
}));

const { data, pending, refresh } = await useAdminFetch<{ data: TourVendor[]; meta: any }>("/api/admin/tour/vendors", { query });
...
function openCreate() {
  Object.assign(form, { id: null, name: "", vendorType: "HOTEL", contactName: "", whatsapp: "", email: "", city: "", country: "", defaultCurrency: "IDR", paymentInfo: "", status: "ACTIVE", notes: "" });
  showForm.value = true;
}
function openEdit(v: TourVendor) {
  Object.assign(form, {
    id: v.id,
    name: v.name,
...
async function submit() {
  ...
  try {
    ...
    if (form.id) await adminPatch(`/api/admin/tour/vendors/${form.id}`, body);
    else await adminPost("/api/admin/tour/vendors", body);
...
async function remove(v: TourVendor) {
  if (!confirm(`Hapus vendor ${v.name}?`)) return;
  await adminDelete(`/api/admin/tour/vendors/${v.id}`).catch(()=>{});
  await refresh();
}
</script>
...
<template>
  <div>
    <PageHead title="Vendors" subtitle="Manajemen vendor — Hotel, Transport, Visa, Flight, dll. Kode VND-0001, filter type/status.">
      <template #actions>
        <button type="button" class="min-h-[40px] rounded-xl bg-sht-olive px-4 py-2 text-sm font-semibold text-white" @click="openCreate">+ Tambah Vendor</button>
      </template>
    </PageHead>
    ...
    <div class="mt-6 flex flex-wrap gap-3">
      ...
      <select v-model="vendorType" class="min-h-[44px] rounded-xl border border-neutral-line px-3 text-sm" @change="page=1"><option value="">Semua Tipe</option><option v-for="t in VENDOR_TYPES" :key="t" :value="t">{{ t }}</option></select>
    ...
    <div v-if="showForm" class="mt-6 rounded-2xl border border-neutral-line bg-white p-6">
      ...
      <div class="mt-4 grid gap-3 sm:grid-cols-2">
        ...
        <label class="text-sm font-medium">Tipe<select v-model="form.vendorType" class="mt-1 min-h-[44px] w-full rounded-xl border border-neutral-line px-3"><option v-for="t in VENDOR_TYPES" :key="t" :value="t">{{ t }}</option></select></label>
    ...
    <div class="mt-6 overflow-x-auto rounded-2xl border border-neutral-line bg-white">
      <table class="w-full min-w-[900px] text-left text-sm">
        <thead class="border-b bg-neutral-warm text-xs uppercase text-neutral-charcoal/60"><tr><th class="px-5 py-3">Kode</th><th class="px-5 py-3">Nama</th><th class="px-5 py-3">Tipe</th><th class="px-5 py-3">Kontak</th><th class="px-5 py-3">Currency</th><th class="px-5 py-3">Status</th><th class="px-5 py-3 text-right">Aksi</th></tr></thead>
        <tbody class="divide-y">
          <tr v-for="v in rows" :key="v.id">
            <td class="px-5 py-3 font-mono text-xs font-semibold">{{ v.vendorCode }}</td>
            <td class="px-5 py-3 font-medium">{{ v.name }}<p class="text-xs text-neutral-charcoal/50">{{ v.city }} {{ v.country ? "· "+v.country : "" }}</p></td>
            <td class="px-5 py-3"><span class="rounded-full bg-neutral-warm px-2.5 py-1 text-xs font-semibold">{{ v.vendorType }}</span></td>
            ...
            <td class="px-5 py-3"><span class="rounded-full px-2.5 py-1 text-xs font-semibold" :class="v.status==='ACTIVE' ? 'bg-sht-olive/10 text-brand-green' : 'bg-neutral-warm text-neutral-charcoal/50'">{{ v.status }}</span></td>
            <td class="px-5 py-3 text-right">
              <button class="rounded-lg px-3 py-1.5 text-xs font-semibold text-brand-teal hover:bg-sht-olive/5" @click="openEdit(v)">Edit</button>
              <NuxtLink :to="`/tour/vendors/${v.id}`" class="rounded-lg px-3 py-1.5 text-xs font-semibold text-neutral-charcoal/60 hover:text-brand-green">Detail</NuxtLink>
              <button class="rounded-lg px-3 py-1.5 text-xs font-semibold text-neutral-charcoal/50 hover:text-red-600" @click="remove(v)">Hapus</button>
            </td>
          </tr>
          <tr v-if="rows.length===0"><td colspan="7" class="px-5 py-10 text-center text-neutral-charcoal/50">Belum ada vendor.</td></tr>
        </tbody>
      </table>
    </div>
