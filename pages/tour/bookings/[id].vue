    ...
    <div v-if="booking" class="mt-6 grid gap-6 lg:grid-cols-3">
      ...
      <div class="lg:col-span-1 space-y-6">
        <div class="rounded-2xl border border-neutral-line bg-white p-6">
          ...
          <p class="mt-1 text-xs text-neutral-charcoal/60">Satu pemesanan mewakili satu layanan vendor untuk Order / Trip.</p>
            ...
            <div>
              ...
              <dd class="mt-1">
                <NuxtLink :to="`/tour/vendors/${booking.vendorId}`" class="font-medium text-brand-teal hover:underline">
                  {{ booking.vendor?.vendorCode ? `${booking.vendor.vendorCode} · ` : '' }}{{ booking.vendor?.name || `Vendor #${booking.vendorId}` }}
                </NuxtLink>
                <p v-if="booking.vendor" class="text-xs text-neutral-charcoal/60">{{ booking.vendor ? vendorTypeLabel((booking.vendor as any).vendorType || booking.bookingType) : '' }}</p>
            ...
            <div>
              ...
              <dd class="mt-1">
                <NuxtLink v-if="booking.order" :to="`/tour/orders/${booking.orderId}`" class="font-medium text-brand-teal hover:underline">
                  {{ booking.order.orderCode }} · {{ booking.customer?.name || `Customer #${booking.order.customerId || booking.customer?.id}` }}
                </NuxtLink>
                <span v-else class="text-neutral-charcoal/50">{{ booking.orderId ? `Order #${booking.orderId}` : "Tanpa Order" }}</span>
                <p v-if="booking.customer" class="text-xs text-neutral-charcoal/60">{{ booking.customer.customerCode }} · {{ booking.customer.name }}</p>
              </dd>
            </div>
          </dl>
        </div>
        ...
        <div class="rounded-2xl border border-neutral-line bg-white p-6">
          ...
          <div class="mt-6 rounded-xl bg-neutral-warm/60 px-4 py-3 text-xs text-neutral-charcoal/60">
            Lead → Customer → Order → (Jamaah, Trips, Bookings → Vendors). Jamaah dikelola di Order Detail, bukan di sini.
          </div>
        </div>
      </div>
      ...
      <div class="lg:col-span-2">
        <div class="rounded-2xl border border-neutral-line bg-white p-6">
          <h3 class="font-heading text-sm font-semibold">Ringkasan Pemenuhan</h3>
          <p class="mt-1 text-xs text-neutral-charcoal/60">Booking ini adalah catatan pemenuhan vendor. Untuk data jamaah, buka Order terkait.</p>

          <div class="mt-6 grid gap-4 sm:grid-cols-3">
            <div class="rounded-xl border border-neutral-line bg-neutral-warm/40 p-4">
              <p class="text-[11px] uppercase tracking-wide text-neutral-charcoal/50">Vendor</p>
              <p class="mt-1 text-sm font-medium">{{ booking.vendor?.name || `Vendor #${booking.vendorId}` }}</p>
              <p class="text-xs text-neutral-charcoal/60">{{ booking.vendor?.vendorCode || '' }}</p>
            </div>
            <div class="rounded-xl border border-neutral-line bg-neutral-warm/40 p-4">
              <p class="text-[11px] uppercase tracking-wide text-neutral-charcoal/50">Trip</p>
              <p class="mt-1 text-sm font-medium">{{ booking.trip ? `${booking.trip.tripCode} · ${booking.trip.name}` : 'Tanpa Trip' }}</p>
            </div>
            <div class="rounded-xl border border-neutral-line bg-neutral-warm/40 p-4">
              <p class="text-[11px] uppercase tracking-wide text-neutral-charcoal/50">Order</p>
              <p class="mt-1 text-sm font-medium">{{ booking.order ? booking.order.orderCode : (booking.orderId ? `Order #${booking.orderId}` : 'Tanpa Order') }}</p>
              <p class="text-xs text-neutral-charcoal/60">{{ booking.customer?.name || '' }}</p>
              <NuxtLink v-if="booking.orderId" :to="`/tour/orders/${booking.orderId}`" class="mt-2 inline-flex text-xs font-semibold text-brand-teal hover:underline">Buka Order →</NuxtLink>
            </div>
          </div>

          <div class="mt-6 rounded-xl border border-dashed border-neutral-line p-6 text-center text-sm text-neutral-charcoal/50">
            Jamaah tidak dikelola di Booking Detail. Satu Order memiliki banyak Jamaah, banyak Bookings, dan beberapa Trips.
            <br/>
            <NuxtLink v-if="booking.orderId" :to="`/tour/orders/${booking.orderId}`" class="mt-3 inline-block rounded-xl bg-sht-olive px-4 py-2 text-xs font-semibold text-white">Kelola Jamaah di Order Detail</NuxtLink>
          </div>
        </div>
      </div>
    </div>
