...
const { data, refresh } = await useAdminFetch<{ data: TourBooking[]; meta: any }>("/api/admin/tour/bookings", { query });
...
async function submit() {
  ...
  try {
    ...
    const snapshot = form.exchangeRateSnapshot ? Number(form.exchangeRateSnapshot) : null;
    let amountIdr = amount;
    if (form.currency !== "IDR" && snapshot) amountIdr = amount * snapshot;
    const body: any = {
...
async function remove(b: TourBooking) {
...
<template>
  <div>
    <PageHead title="Bookings" subtitle="Booking vendor per Trip/Order — snapshot kurs, amountIdr tidak recalc saat global rate berubah.">
    ...
    <div class="mt-6 flex flex-wrap gap-3">
      ...
      <select v-model="bookingType" class="min-h-[44px] rounded-xl border border-neutral-line px-3 text-sm" @change="page=1"><option value="">Semua Tipe</option><option v-for="t in TYPES" :key="t" :value="t">{{ t }}</option></select>
    ...
    <div v-if="showForm" class="mt-6 rounded-2xl border border-neutral-line bg-white p-6">
      ...
      <div class="mt-4 grid gap-3 sm:grid-cols-2">
        ...
        <label class="text-sm font-medium">Order (opsional)<select v-model="form.orderId" class="mt-1 min-h-[44px] w-full rounded-xl border border-neutral-line px-3"><option value="">Tanpa Order</option><option v-for="o in orders" :key="o.id" :value="o.id">{{ o.orderCode }} · {{ o.paxCount }} pax</option></select></label>
        <label class="text-sm font-medium">Tipe<select v-model="form.bookingType" class="mt-1 min-h-[44px] w-full rounded-xl border border-neutral-line px-3"><option v-for="t in TYPES" :key="t" :value="t">{{ t }}</option></select></label>
    ...
    <div class="mt-6 overflow-x-auto rounded-2xl border border-neutral-line bg-white">
      <table class="w-full min-w-[1100px] text-left text-sm">
        <thead class="border-b bg-neutral-warm text-xs uppercase text-neutral-charcoal/60"><tr><th class="px-5 py-3">Kode</th><th class="px-5 py-3">Tanggal</th><th class="px-5 py-3">Vendor</th><th class="px-5 py-3">Tipe</th><th class="px-5 py-3">Trip/Order</th><th class="px-5 py-3">Amount</th><th class="px-5 py-3">IDR</th><th class="px-5 py-3">Status</th><th class="px-5 py-3 text-right">Aksi</th></tr></thead>
        <tbody class="divide-y">
          <tr v-for="b in rows" :key="b.id">
            <td class="px-5 py-3 font-mono text-xs font-semibold">{{ b.bookingCode }}</td>
            <td class="px-5 py-3 text-xs">{{ b.bookingDate }}</td>
            <td class="px-5 py-3 text-xs">#{{ b.vendorId }}</td>
            <td class="px-5 py-3"><span class="rounded-full bg-neutral-warm px-2 py-1 text-xs">{{ b.bookingType }}</span></td>
            <td class="px-5 py-3 text-xs">{{ b.tripId ? "Trip #"+b.tripId : "—" }} / {{ b.orderId ? "Order #"+b.orderId : "—" }}</td>
            ...
            <td class="px-5 py-3"><span class="rounded-full px-2.5 py-1 text-xs font-semibold" :class="b.status==='CONFIRMED' ? 'bg-sht-olive/10 text-brand-green' : 'bg-neutral-warm text-neutral-charcoal/60'">{{ b.status }}</span></td>
          ...
          <tr v-if="rows.length===0"><td colspan="9" class="px-5 py-10 text-center text-neutral-charcoal/50">Belum ada booking.</td></tr>
        </tbody>
      </table>
    </div>
