...
const { data, refresh } = await useAdminFetch<{ data: ExchangeRate[] }>(
...
async function submit() {
    ...
    error.value = null;
    try {
        await adminPost("/api/admin/exchange-rates", {
...
async function deactivate(r: ExchangeRate) {
    if (!confirm(`Nonaktifkan kurs ${r.sourceCurrency}→${r.targetCurrency}?`))
        return;
...
<template>
    <div>
        <PageHead
            ...
            subtitle="Kurs dikelola admin (tanpa API eksternal). Kurs aktif di-snapshot ke setiap estimasi."
        >
            <template #actions>
                <button
                    type="button"
        ...
        >
            ...
            <p class="mt-1 text-xs text-neutral-charcoal/60">
                Menambah kurs untuk pasangan yang sama otomatis menonaktifkan
                kurs lama (satu kurs aktif per pasangan).
            </p>
            ...
            <table class="w-full min-w-[560px] text-left text-sm">
                ...
                >
                    <tr>
                        ...
                        <th class="px-5 py-3">Berlaku sejak</th>
                ...
                <tbody class="divide-y divide-neutral-line">
                    <tr v-for="r in rows" :key="r.id">
                        ...
                        <td class="px-5 py-3 text-neutral-charcoal/60">
                            {{
                                new Date(r.effectiveAt).toLocaleString("id-ID")
                            }}
                        </td>
                        <td class="px-5 py-3">
                            <span
