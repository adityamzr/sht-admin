...
const { data, refresh } = await useAdminFetch<{ data: DepartureCity[] }>(
    "/api/admin/departure-cities",
);
...
async function submit() {
    ...
    try {
        ...
        if (form.id)
            await adminPatch(`/api/admin/departure-cities/${form.id}`, body);
...
async function toggle(c: DepartureCity) {
    await adminPatch(`/api/admin/departure-cities/${c.id}`, {
        isActive: !c.isActive,
    }).catch(() => {});
    await refresh();
}
...
<template>
    <div>
        <PageHead
            ...
            subtitle="Kota keberangkatan & fee per jamaah (berpengaruh ke pricing)."
        >
            <template #actions>
                <button
                    type="button"
        ...
        >
            ...
            <div class="mt-4 grid gap-3 sm:grid-cols-2">
                ...
                <label class="text-sm font-medium"
                    >Fee per pax (kosongkan bila 0)
                    <input
                        v-model="form.feePerPax"
                        type="number"
                        min="0"
                        class="mt-1 min-h-[44px] w-full rounded-xl border border-neutral-line px-4"
                        placeholder="650000"
                ...
                <tbody class="divide-y divide-neutral-line">
                    <tr v-for="c in rows" :key="c.id">
                        ...
                        <td class="px-5 py-3">
                            {{
                                c.feePerPax
                                    ? `${c.feePerPax.toLocaleString("id-ID")} ${c.feeCurrency}`
                                    : "—"
                            }}
                        </td>
                        <td class="px-5 py-3">
