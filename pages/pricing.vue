...
const { data, refresh } = await useAdminFetch<{ data: PricingRecord[] }>(
    "/api/admin/pricing",
);
const { data: hotelsData } = await useAdminFetch<{ data: Hotel[] }>(
    "/api/admin/hotels",
);
const { data: flightsData } = await useAdminFetch<{ data: Flight[] }>(
    "/api/admin/flights",
);
const { data: routesData } = await useAdminFetch<{ data: TransportRoute[] }>(
    "/api/admin/routes",
);
const { data: servicesData } = await useAdminFetch<{ data: Service[] }>(
    "/api/admin/services",
);
const { data: periodsData } = await useAdminFetch<{ data: PricingPeriod[] }>(
    "/api/admin/pricing-periods",
);

const rows = computed(() => data.value?.data ?? []);
const periods = computed(() =>
    (periodsData.value?.data ?? []).filter((p) => p.isActive),
);
...
const entityOptions = computed<Record<string, EntityOption[]>>(() => ({
    hotel_room_type: (hotelsData.value?.data ?? []).flatMap((h) =>
        h.roomTypes.map((r) => ({
            value: String(r.id),
            label: `${h.name} — ${r.name} (kap. ${r.capacity})`,
        })),
    ),
    flight: (flightsData.value?.data ?? []).map((f) => ({
        value: String(f.id),
        label: `${f.airline} · ${f.routeLabel}`,
    })),
    route_vehicle: (routesData.value?.data ?? []).flatMap((r) =>
        r.vehicleOptions.map((o) => ({
            value: String(o.id),
            label: `${r.name} · ${o.vehicle?.name ?? "#" + o.vehicleId}`,
        })),
    ),
    service: (servicesData.value?.data ?? []).map((s) => ({
        value: String(s.id),
        label: `${s.name} (${s.pricingUnit})`,
    })),
}));
...
function onEntityTypeChange() {
    form.pricingUnit =
        form.entityType === "hotel_room_type"
            ? "room_night"
            : form.entityType === "flight"
              ? "pax"
              : form.entityType === "route_vehicle"
                ? "vehicle_trip"
                : "pax";
}
async function submit() {
    ...
    try {
        const body = {
            ...
            supplierCost:
                form.supplierCost === "" ? null : Number(form.supplierCost),
            markupType: form.strategy === "manual" ? null : form.markupType,
            markupValue:
                form.markupValue === "" ? null : Number(form.markupValue),
            sellingPrice:
                form.sellingPrice === "" ? null : Number(form.sellingPrice),
...
async function toggle(r: PricingRecord) {
    await adminPatch(`/api/admin/pricing/${r.id}`, {
        isActive: !r.isActive,
    }).catch(() => {});
    await refresh();
}
async function remove(r: PricingRecord) {
    if (
        !confirm(
            "Hapus pricing record ini? (estimasi historis aman — snapshot menyimpan salinan)",
        )
    )
        return;
...
<template>
    <div>
        <PageHead
            title="Pricing"
            subtitle="Satu arsitektur harga untuk semua entitas: strategi, periode, mata uang. Data supplier cost/markup = INTERNAL."
        >
            <template #actions>
                <button
                    type="button"
        ...
        >
            ...
            <div class="mt-4 grid gap-3 sm:grid-cols-3">
                <label class="text-sm font-medium"
                    ...
                    >
                        <option
                            v-for="(label, key) in entityLabel"
                            :key="key"
                            :value="key"
                        >
                            {{ label }}
                ...
                <label class="text-sm font-medium"
                    ...
                    >
                        ...
                        <option
                            v-for="o in entityOptions[form.entityType]"
                            :key="o.value"
                            :value="o.value"
                        >
                            {{ o.label }}
                ...
                <label class="text-sm font-medium"
                    ...
                    >
                        <option
                            v-for="(label, key) in strategyLabel"
                            :key="key"
                            :value="key"
                        >
                            {{ label }}
                ...
                <label class="text-sm font-medium sm:col-span-2"
                    ...
                    <input
                        ...
                        class="mt-1 min-h-[44px] w-full rounded-xl border border-neutral-line px-4"
                        placeholder="opsional — tidak pernah tampil publik"
                    />
                </label>
            </div>
            <p
            ...
            <table class="w-full min-w-[860px] text-left text-sm">
                ...
                >
                    <tr>
                        ...
                        <th class="px-5 py-3">Strategi</th>
                        <th class="px-5 py-3">Internal</th>
                    ...
                    <tr v-for="r in rows" :key="r.id">
                        <td class="px-5 py-3">
                            ...
                            <p class="text-xs text-neutral-charcoal/50">
                                periode #{{ r.periodId }} · {{ r.currency }}
                            </p>
                        </td>
                        <td class="px-5 py-3 text-neutral-charcoal/60">
                            {{ strategyLabel[r.strategy] ?? r.strategy }}
                        </td>
                        <td class="px-5 py-3 text-neutral-charcoal/50">
                            <template v-if="r.strategy === 'manual'"
                                >—</template
                            >
                            <template v-else>
                                cost {{ fmt(r.supplierCost) }}
                                <template v-if="r.markupType === 'percentage'"
                                    >+ {{ r.markupValue }}%</template
                                ><template v-else
                                    >+ {{ fmt(r.markupValue) }}</template
                                >
                            </template>
                        </td>
                        <td class="px-5 py-3 font-semibold text-brand-green">
                            {{ fmt(r.computedSellingPrice) }}
                        </td>
                        <td class="px-5 py-3 text-neutral-charcoal/60">
                            {{ r.pricingUnit }}
                        </td>
