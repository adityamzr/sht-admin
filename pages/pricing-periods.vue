...
const { data, refresh } = await useAdminFetch<{ data: PricingPeriod[] }>(
...
async function submit() {
    ...
    try {
        ...
        if (form.id)
            await adminPatch(`/api/admin/pricing-periods/${form.id}`, body);
...
async function toggle(p: PricingPeriod) {
    await adminPatch(`/api/admin/pricing-periods/${p.id}`, {
        isActive: !p.isActive,
    }).catch(() => {});
    await refresh();
}
...
<template>
    <div>
        <PageHead
            ...
            subtitle="Prioritas numerik unik: saat beberapa periode mencakup tanggal yang sama, prioritas tertinggi menang."
        >
            <template #actions>
                <button
                    type="button"
        ...
        >
            <h3 class="font-heading text-base font-semibold">
                {{ form.id ? "Edit Periode" : "Tambah Periode" }}
            </h3>
            ...
            <table class="w-full min-w-[640px] text-left text-sm">
                ...
                <tbody class="divide-y divide-neutral-line">
                    <tr v-for="p in rows" :key="p.id">
                        ...
                        <td class="px-5 py-3">
                            <span
                                class="rounded-full bg-brand-sky/50 px-2.5 py-1 text-xs font-bold text-brand-green"
                                >{{ p.priority }}</span
                            >
                        </td>
                        <td class="px-5 py-3">
