...
export default defineEventHandler(async(e)=>{const b=await readValidatedBody(e,input.safeParse);if(!b.success)throw createError({statusCode:400,statusMessage:'Bulk status tidak valid.'});const db=useDb();const rows=await db.update(articles).set({status:b.data.status,publishedAt:b.data.status==='PUBLISHED'?sql`coalesce(${articles.publishedAt}, now())`:null,updatedAt:new Date()}).where(inArray(articles.id,b.data.ids)).returning({id:articles.id});return {requested:b.data.ids.length,updated:rows.length,failed:b.data.ids.length-rows.length,ids:rows.map(r=>r.id)}})
