...
export default defineEventHandler(async (event) => {
  ...
  if (parsed.data.checkInDate) patch.checkInDate = (parsed.data.checkInDate as Date).toISOString().slice(0,10)
  if (parsed.data.checkOutDate) patch.checkOutDate = (parsed.data.checkOutDate as Date).toISOString().slice(0,10)
