...
import { updateTourInvoice, getTourWorkspaceIdFinance } from '~/server/services/tour-finance'
import { tourInvoicePatch } from '~/server/utils/tour-validators'
import { adminTourInvoice } from '~/server/utils/tour-serializers'

export default defineEventHandler(async (event) => {
  const id = Number(getRouterParam(event, 'id'))
  const body = await readValidatedBody(event, tourInvoicePatch.safeParse)
  ...
  const row = await updateTourInvoice(db, id, workspaceId, body.data, user?.id)
