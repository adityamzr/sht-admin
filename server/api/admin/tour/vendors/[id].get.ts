...
import { getTourVendor, getTourWorkspaceId, listTourBookings } from '~/server/services/tour-operations'
...
export default defineEventHandler(async (event) => {
  ...
  const row = await getTourVendor(db, id, workspaceId)
  ...
  return { data: { ...adminTourVendor(row), bookings: bookings.data } }
})
